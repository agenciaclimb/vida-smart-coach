...
begin
  if not exists (
    ...
    where schemaname='public' and tablename='whatsapp_messages' and policyname='Service role can manage whatsapp_messages'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname='public' and tablename='whatsapp_messages' and policyname='Users can view whatsapp_messages'
  ) then
