...
-- Data: 27/10/2025
-- Objetivo: Consolidar pontuação de todas as fontes para uso consistente
--           em header, ranking, gráficos e relatórios
-- ==========================================

-- Criar view que consolida XP de múltiplas fontes
...
SELECT
  ...
  COALESCE(g.current_level, 0) AS current_level_legacy,
...
-- (Views herdam as RLS policies das tabelas base, mas reforçamos aqui)
ALTER VIEW public.v_user_xp_totals OWNER TO postgres;

-- Índices nas tabelas base para otimizar a view
-- (já existem na maioria, mas garantimos)
