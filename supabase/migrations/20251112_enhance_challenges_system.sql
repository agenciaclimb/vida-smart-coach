-- Migration: 20251112_enhance_challenges_system.sql
-- Adiciona stored procedures e achievements específicos para desafios
...
BEGIN
  ...
  -- Atualizar level baseado em total_points
  UPDATE gamification
...
AS $$
BEGIN
  -- Quando um novo usuário é criado, inscrevê-lo automaticamente em desafios ativos
...
-- Trigger para auto-inscrição (opcional - apenas para novos usuários)
DROP TRIGGER IF EXISTS auto_join_challenges_trigger ON auth.users;
-- Comentado por segurança - habilitar apenas se desejar auto-inscrição
-- CREATE TRIGGER auto_join_challenges_trigger
-- AFTER INSERT ON auth.users
-- FOR EACH ROW
-- EXECUTE FUNCTION auto_join_active_challenges();
...
-- RLS policy para a view
ALTER VIEW user_active_challenges SET (security_invoker = on);
...
COMMENT ON FUNCTION add_user_xp IS 'Adiciona XP ao usuário e atualiza automaticamente o level baseado em total_points / 1000';
COMMENT ON FUNCTION auto_join_active_challenges IS 'Auto-inscreve novos usuários em desafios ativos (trigger opcional)';
COMMENT ON FUNCTION expire_old_challenges IS 'Desativa desafios expirados - deve ser chamado por cron job';
COMMENT ON VIEW user_active_challenges IS 'Visão consolidada dos desafios ativos de cada usuário com progresso e tempo restante';
