...
function ProductDetailPage() {
  ...
  const handleAddToCart = useCallback(async () => {
    if (product && selectedVariant) {
      ...
      try {
        ...
        toast.success(`${quantity} x ${product.title} (${selectedVariant.title}) adicionado ao carrinho!`);
      } catch (error) {
      ...
      return (
        <div className="max-w-5xl mx-auto py-12">
          ...
          <div className="text-center text-red-400 p-8 glass-card rounded-2xl">
            ...
            <p className="mb-6 text-gray-300">O produto que você está procurando não existe ou foi removido.</p>
            <Button onClick={() => navigate('/store')} className="bg-purple-500 hover:bg-purple-600 text-white">
  ...
  return (
    <>
      <Helmet>
        <title>{loading || !product ? 'Carregando...' : `${product.title} - Loja Vida Smart`}</title>
        <meta name="description" content={loading || !product ? 'Carregando produto' : product.description?.substring(0, 160) || product.title} />
      </Helmet>
